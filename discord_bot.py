...
# 📝 加上全局攔截器：確保資料庫完全就緒後，且「本機為主節點」時，才允許使用者下達指令
@bot.check
async def globally_block_before_ready(ctx):
    ...
    if not has_lock:
        try:
            await ctx.send("⏸️ 目前由其他主要機器人服務中，本機為「待機接管模式」，暫不接受指令。")
        except Exception:
            pass
        return False
...
# 📝 新增這兩個變數：處理待機接管邏輯
has_lock = False             # 標記本機目前是否為「主要服務節點」(有沒有搶到鎖)
standby_task_started = False # 防止斷線重連時重複產生待機迴圈
...
async def maintain_lock_task():
    """每 10 秒更新一次心跳鎖，並檢查是否發生腦裂"""
    try:
        result = await col_lock.update_one(
            {"_id": "global_bot_lock", "holder": INSTANCE_ID},
            {"$set": {"heartbeat": time.time()}}
        )
        
        # 🛡️ 腦裂防護：如果更新筆數是 0，代表資料庫裡的 holder 已經不是我的 INSTANCE_ID
        # 這通常發生在「本機網路斷線超過 30 秒，鎖被備用機搶走」的狀況
        if result.matched_count == 0:
            print("💥 [致命] 雲端鎖已被其他實例接管！為防止雙主腦衝突，本機將自動關閉...")
            await bot.close() # 關閉後交由 PM2 / Docker 自動重啟，重啟後會以乾淨狀態進入待機模式
            
    except Exception as e: 
        print(f"⚠️ 維持心跳鎖發生異常: {e}")

# --- 🔄 待機接管邏輯 ---
async def standby_loop():
    """每隔一段時間檢查是否能接管主節點"""
    global tasks_started, has_lock
    ...
    while not bot.is_closed():
        ...
        if is_locked:
            has_lock = True
            print(f"👑 成功獲取雲端鎖 (ID: {INSTANCE_ID})！正式接管並啟動背景服務...")
            ...
            has_lock = False
            print("⏳ 另一個機器人實例正在運行中。進入待機模式，60秒後重新檢查...")
            # 更改狀態為閒置，讓你知道這隻 Bot 活著但在待機
            await bot.change_presence(status=discord.Status.idle, activity=discord.Game(name="⏳ 待機中 | 隨時準備接管"))
            await asyncio.sleep(60) # 每 60 秒醒來檢查一次
...
async def on_ready():
    ...
    # 📝 啟動待機與接管迴圈 (取代原本失敗就直接 bot.close() 的設計)
    global standby_task_started
    if not standby_task_started:
        bot.loop.create_task(standby_loop())
        standby_task_started = True
